  ...
  if (typeof body.uploadToken === "string" && body.uploadToken) {
    try {
      if (typeof body.attachmentToken === "string" && body.attachmentToken) {
        const { data, error } = await createAdminClient().rpc("vortex_get_pending_public_attachment", { p_upload_token: body.uploadToken, p_attachment_token: body.attachmentToken });
        const pending = data?.[0] as { storage_path?: string; mime_type?: string; size_bytes?: number } | undefined;
        if (error || !pending?.storage_path || pending.mime_type !== file.value.mimeType || pending.size_bytes !== file.value.size) return NextResponse.json({ error: "Não foi possível autorizar o anexo." }, { status: 403 });
      ...
      const { data, error } = await createAdminClient().rpc("vortex_reserve_public_attachment_upload", { p_upload_token: body.uploadToken, p_original_name: file.value.fileName, p_mime_type: file.value.mimeType, p_file_size: file.value.size });
      const reservation = data?.[0] as { storage_path?: string; attachment_token?: string } | undefined;
      if (error || !reservation?.storage_path || !reservation.attachment_token) return NextResponse.json({ error: "Não foi possível autorizar o anexo." }, { status: 403 });
    ...
    } catch { return NextResponse.json({ error: "Não foi possível preparar o envio do anexo." }, { status: 503 }); }
  }
